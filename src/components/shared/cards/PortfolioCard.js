...
const PortfolioCard = ({ portfolio }) => {
  ...
  const { title, img, shortDesc, id, dataFilter, link } = portfolio
    ? portfolio
    : {};
  const isLight = themeMode === "light" ? true : false;
  return (
    <div
      className={`portfolio-item ${dataFilter} bg-primary-color-light pb-0 lg:pb-0 rounded-10px group relative float-left inline-flex`}
    >
      <Image
        src={img}
        alt=""
